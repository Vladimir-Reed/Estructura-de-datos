...
bool actualizarTiempo(chrono::steady_clock::time_point& inicio)
{
    auto ahora = chrono::steady_clock::now();
    int transcurrido = (int)chrono::duration_cast<chrono::seconds>(ahora - inicio).count();

    if (transcurrido > 0)
    {
        if (turnoActual == 1)
            tiempoRojas -= transcurrido;
        else
            tiempoBlancas -= transcurrido;

        if (tiempoRojas < 0) tiempoRojas = 0;
        if (tiempoBlancas < 0) tiempoBlancas = 0;

        inicio = ahora;

        if (tiempoRojas == 0 || tiempoBlancas == 0)
        {
            tiempoAgotado = true;
            return false;
        }
...
void mostrarHistorial()
{
    ...
    else
    {
        string linea;
        int numeroPartida = 0;
        bool partidaAbierta = false;

        while (getline(archivo, linea))
        {
            if (linea == "PARTIDA NUEVA")
            {
                numeroPartida++;
                partidaAbierta = true;
    ...
    cout << "\n------------------------------------\n";
    cout << "Las partidas se muestran en forma de lista.\n";
    cout << "------------------------------------\n";
    system("pause");
}
    ...
    while (jugando)
    {
        ...
        if (!seleccionarFicha(fila, columna))
        {
            if (tiempoAgotado)
            {
                imprimirTablero();
                if (turnoActual == 1)
                    cout << "\nSe acabo el tiempo de Rojas. GANAN LAS BLANCAS.\n";
                else
                    cout << "\nSe acabo el tiempo de Blancas. GANAN LAS ROJAS.\n";
            }
            else
            {
                cout << "\nPartida abandonada.\n";
...
int main()
{
    ...
    do
    {
        ...
        cout << "3. Historial de partidas (lista visible)\n";
